...
class Settings2Screen(GridScreen):
    ...
    def __init__(self):
        ...
        super().__init__([
            ...
            Text("settings"), None, Text("profiles")
        ], GRID_4x3, start=1)
...
profile_alphabet = [c for c in "abcdefghijklmnopqrstuvwxyz "]
...
alphabet = [c for c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ"] + [
    ...
    "SHIFT", "RIGHT_SHIFT", "DELETE", "CONTROL", "RIGHT_CONTROL"
]
class InputScreen(GridScreen):
    ...
    def __init__(self, switch_no):
        ...
        self.keys = config.data.keys[self.switch_no - 1] if self.switch_no <= 6 else config.data.enc_keys[self.switch_no - 7]
        self.key_index = 0
        self.char_index = alphabet.index(self.keys[self.key_index] if not self.has_duration else self.keys[self.key_index][0])

        def on_save():
            if not self.has_duration:
                config.data.keys[self.switch_no - 1] = self.keys
            else:
                config.data.enc_keys[self.switch_no - 7] = self.keys
            config.save()
        ...
        if not self.has_duration:
            self.alphabet = Select(alphabet, alphabet.index(self.keys[self.key_index]), on_change=on_alphabet_change)
            super().__init__([
            ...
            self.alphabet = Select(alphabet, alphabet.index(self.keys[self.key_index][0]), on_change=on_alphabet_change)
...
class ColorSelectScreen(GridScreen):
    ...
    def __init__(self, switch_no = 1):
        ...
        switch_no = NumSelect(self.switch_no, 1, 5, format=lambda v: f"edit {types[v]} colors", on_change=lambda v: setattr(self, "switch_no", v))
